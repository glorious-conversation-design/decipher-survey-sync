...
const decipher_survey_sync =  {
    ...
    download_survey: async function (api_key, the_project_number = null) {
        ...
        try {
            ...
            const idMatcher = /<(?:html|select|number|float|text|textarea|radio|checkbox|row|col|choice|suspend|invalid|completed|samplesource|title|block|loop|var|exit|goto|res)[^>]*(\sid="\w+")/gsi;
            const ids = [];
            let idnow;
            try {
